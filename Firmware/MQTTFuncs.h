...
void MQTTUnSubscribe()
{
    String topicN = String("smart-adc-device/deviceExistance");
...
void callback(char *topic, byte *payload, unsigned int length)
{
    ...
    Serial.println();
    if (String(topic) == String("smart-adc-device/deviceExistance"))
    {
...
bool mqttConnect()
{
    ...
    while (!mqttClient.connected())
    {
        ...
        Serial.println(String("Attempting MQTT broker:") + String("Smart ADC Broker"));
